...
from rest_framework import status
from django.utils.dateparse import parse_datetime
from .models import Appointment, Availability, Doctor, Patient
from billing.models import Payment
...
class BookAppointmentView(APIView):
    def post(self, request):
        ...
        appointment_date = data.get("appointment_date")  # تاريخ الموعد
        appointment_time = data.get("appointment_time")  # وقت الموعد
        payment_status = data.get("payment_status")  # حالة الدفع
        payment_transaction_id = data.get("payment_transaction_id")  # معرف المعاملة من بوابة الدفع
        ...
        # التحقق من الدفع قبل الحجز
        if not payment_status or payment_status != "paid":
            return Response({"error": "Payment is required to book the appointment."}, status=status.HTTP_400_BAD_REQUEST)

        # التحقق من وجود معلومات الدفع
        if not payment_transaction_id:
            return Response({"error": "Payment transaction ID is missing."}, status=status.HTTP_400_BAD_REQUEST)

        # التحقق من الدفع (إضافة عملية تحقق من قاعدة بيانات الدفع إذا لزم الأمر)
        try:
            payment = Payment.objects.get(transaction_id=payment_transaction_id, status='success')
        except Payment.DoesNotExist:
            return Response({"error": "Payment not found or payment was not successful."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        appointment = Appointment.objects.create(
            ...
            payment=payment,  # ربط الدفع بالموعد
            status='booked'  # تحديث حالة الموعد ليكون محجوز
        )
