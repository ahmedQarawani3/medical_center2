...
class BookAppointmentView(APIView):
    def post(self, request):
        ...
        appointment_datetime = parse_datetime(f"{appointment_date}T{appointment_time}")  
        if not appointment_datetime:
            return Response({"error": "Invalid appointment time."}, status=status.HTTP_400_BAD_REQUEST)

        availability = Availability.objects.filter(
            doctor=doctor,
            day=appointment_datetime.strftime("%A"),  
        ...
        if Appointment.objects.filter(
            ...
            time_slot=appointment_datetime.time()  
        ).exists():
        ...
        try:
            ...
            return Response({"error": "User is not registered as a patient."}, status=status.HTTP_400_BAD_REQUESنشاء الموعد
