...
from doctors.models import a
class BookAppointmentView(APIView):
    def post(self, request):
        data = request.data
        doctor_id = data.get("doctor_id")
        appointment_date = data.get("appointment_date")  # تاريخ الموعد
        appointment_time = data.get("appointment_time")  # الوقت بصيغة HH:MM:SS
        ...
        if not appointment_date or not appointment_time:
            return Response({"error": "Invalid appointment date or time."}, status=status.HTTP_400_BAD_REQUEST)

        # البحث عن التوفر
        availability = Availability.objects.filter(
            doctor=doctor,
            day=appointment_date.strftime("%A"),  # تحويل التاريخ لاسم اليوم
